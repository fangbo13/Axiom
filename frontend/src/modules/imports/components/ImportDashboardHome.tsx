...
interface ImportDashboardHomeProps {
  onNewImport: () => void
  onOverwriteImport: (batch: ImportBatch) => void
}

const ImportDashboardHome = ({ onNewImport, onOverwriteImport }: ImportDashboardHomeProps) => {
      ...
      render: (_: any, record: ImportBatch) => (
        <Space>
          ...
          <Button
            size="small"
            icon={<UploadOutlined />}
            onClick={() => onOverwriteImport(record)}
          >
            覆盖导入
          </Button>
          <Button
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <Text type="secondary">查看已入库的导入批次，按期间分组管理，或发起新的导入</Text>
        </div>
        <Button
          type="primary"
          icon={<PlusOutlined />}
      ...
      <Card className="rounded-xl shadow-card">
        {loading ? (
          ...
          <Empty description="暂无已入库的导入记录" className="py-20">
            <Button type="primary" icon={<PlusOutlined />} onClick={onNewImport} className="mt-4">
              去导入
            </Button>
          </Empty>
        ) : (
            ...
            <Card size="small" className="rounded-xl shadow-card">
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                <Button
                  ...
                  onClick={() => {
                    closeDetail()
                    onOverwriteImport(batchDetail.batch)
                  }}
                >
                  覆盖导入
                </Button>
...
export default ImportDashboardHome
